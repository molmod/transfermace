...
def main() -> None:
    ...
    train_loader = torch_geometric.dataloader.DataLoader(
        dataset=[
            data.AtomicData.from_config(config, z_table=z_table, cutoff=model_kwargs['r_max'])
    ...
    delta_MLP = ast.literal_eval(args.delta_MLP)
    assert type(delta_MLP) == list
    model_config = dict(
            delta_MLP=delta_MLP,
            delta_atomic_energies=atomic_energies,
            )
    model = TransferMACE.from_model(base_model, **model_config)
    model.compute_delta = True # train delta only
    model.to(device)
    ...
    param_options = dict(
        params=[
            ...
            {
                "name": "readouts_delta",
                "params": model.readouts_delta.parameters(),
                "weight_decay": args.weight_decay,
            },
        ],
        lr=args.lr,
